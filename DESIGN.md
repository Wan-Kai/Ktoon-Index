...
The category page presents one expanded sleeve rather than a compressed remnant of the homepage stack. At every viewport width its three custom comboboxes sit in a dedicated graphite control band inside the crate's normal flow, separated from the paper by one hairline and 24px of breathing room; controls must never hide behind the raised tab. Every browser renders a DOM listbox as a graphite archive menu with one acid selection marker, without an operating-system popup. Hidden native selects retain the filter values; buttons and options have a 44px minimum target. The control band uses local layer 1 over the isolated paper stage at layer 0, below the masthead search context. Arrow keys preview, Enter confirms, Escape/Tab dismiss without changing the filter. Above 1200px the sleeve targets 286-330px height with 62px catalog rows, a wider identity rail, and more generous paper padding. Between 821px and 1200px the identity rail returns to 280px, rows compact to 58px, and the description column becomes fluid so the layout cannot overflow. At 820px and below, the existing single-column sleeve and compact three-control row remain authoritative.

On the homepage, the identity rail compresses to 280px at 1080px. At 820px the masthead uses a 28px single-line brand wordmark, each sleeve becomes one column, and identity moves above its tracks while the paper stack remains continuously edge-to-edge with no exposed graphite gap. Mobile sleeves keep the same shared outer datum while the local taper reduces to 2px per side at rest and 1px per side when lifted. At 520px the brand wordmark becomes 24px, secondary brand copy and the catalog code disappear, and titles and descriptions wrap rather than truncate. The minimum supported viewport is 320px.
