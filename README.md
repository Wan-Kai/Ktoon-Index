...
M0–M8 已完成：20 个条目由 `content/entries/*.md` 唯一描述；GitHub Actions 每次从事实源重建并校验后部署。CLI 可以通过 `gh auth` 完成单条目的完整 CRUD 与查询，项目 Skill 将自然语言意图稳定映射到同一 CLI，不保存凭据或提供直接写文件的备用路径。M7 完成内容维护第一阶段交付，M8 进一步完成真实浏览器性能硬化与可回归验证。
